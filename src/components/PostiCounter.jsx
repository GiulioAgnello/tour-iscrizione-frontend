...
export default function PostiCounter({ tour }) {
  ...
  const disponibili = posti_disponibili ?? Math.max(0, posti_max - partecipanti_totali);
  const percentuale = Math.min(100, Math.round((partecipanti_totali / posti_max) * 100));
  ...
  return (
    <div className={`posti-banner ${urgencyClass}`} role="status" aria-live="polite">
